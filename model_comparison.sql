{% macro compare_models(model_name) %}
    ...
    {# Create maps of column names to data types, normalizing for case sensitivity #}
    {% set dev_col_map = {} %}
    {% set uat_col_map = {} %}
    
    {% for col in dev_columns %}
        {% do dev_col_map.update({col.name|lower: col.dtype}) %}
    {% endfor %}
    
    {% for col in uat_columns %}
        {% do uat_col_map.update({col.name|lower: col.dtype}) %}
    {% endfor %}
    
    {# Log column mappings #}
    {{ log("DEV columns map: " ~ dev_col_map, info=True) }}
    {{ log("UAT columns map: " ~ uat_col_map, info=True) }}
    ...
    {% set stats_query %}
        ...
        SELECT 
            ...
            {% for column in dev_columns %}
                {% if column.name|lower in uat_col_map %}
